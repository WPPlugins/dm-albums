...
Change log:
	...
	3.1.8
		* Refined upload patch after further testing from Mark L (thank you!)

	3.1.8
		* Added (another) patch to (hopefully) fix some user's issues with albums not appearing when uploading.
		* Updated to 1.2.8 of galleria; this should resolve many of the JQuery issues some users were experiencing

	3.1.7
		* Added patch to (hopefully) fix some user's issues with albums not appearing when uploading.
		* Fixed versioning of javascript libs

	3.1.6
		* Added Flikr support: enter [dmalbum flikr="setid"/] where setid is the id of your set, i.e. [dmalbum flikr="72157629383149390"/]
		* Fixed positioning of photo counter element so it is visible
...
// Global variables
$DM_PHOTOALBUM_APP_VERSION = "3.1.9";
...
function get_galleria($album, $width, $height, $forcesize, $flikr = -1)
{
	...
	<?php if($show_download == "true" && $flikr == -1) {?><div class="dm-albums-download"><div>Download:</div> <a class="fullscreen" href="javascript:void(0);" onClick="g_DM_FULLSCREEN_GALLERY_ID = 'galleria-<?php echo $albumid; ?>'; dm_download_file();"></a></div><?php } ?>
	...
	<script type="text/javascript" class="dm-album-galleria-script" src="<?php echo plugins_url(); ?>/dm-albums/javascript/galleria.php?albumid=<?php echo $albumid; ?>&album=<?php echo $album; ?>&width=<?php echo ($width + 10); ?>&height=<?php echo ($height + 10); ?>&flikr=<?php echo $flikr; ?>"></script>
	<?
...
function dm_albums_embed_shortcode($attr, $content = null)
{
	...
	if(empty($path) && $flikr == -1)	$thecontent = '<div class="dm-albums-fatal-error">Error: DM Albums is missing the required parameter, \'path\'.</div>';
	else								$thecontent = get_galleria($path, $width, $height, $style, $flikr);
